...
logger = logging.getLogger("aea.packages.fetchai.contracts.ercpool")
...
class MyScaffoldContract(Contract):
    ...
    def _get_abi(cls, configuration):
        with open(os.getcwd() + "/contracts/ercpool/" + configuration["contract_interface_paths"]["ethereum"], "r") as f:
    ...
    def get_deploy_transaction(
        ...
        """
        conf = dict(name="ercpool",
                    author="tomrae",
                    version="0.1.0",
                    license_="Apache-2.0",
                    aea_version='>=0.5.0, <0.6.0',
                    contract_interface_paths={
                        'ethereum': 'build/contracts/HegicERCPool.json'}
                    )
    ...
    def _try_estimate_gas(ledger_api: LedgerApi, tx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "[ercpool_contract]: gas estimate: {}".format(gas_estimate))
            tx["gas"] = gas_estimate
        except Exception as e:  # pylint: disable=broad-except
            logger.info(
                "[ercpool_contract]: Error when trying to estimate gas: {}".format(
                    e)
            )
        return tx
