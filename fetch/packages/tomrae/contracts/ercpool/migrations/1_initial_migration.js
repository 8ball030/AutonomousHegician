...
const HedgeContract = artifacts.require("HegicHedgeContract")
...
module.exports = async function (deployer, network) {
  try {
    if (network == "development" || network == "develop") {
      ...
      await deployer.deploy(HedgeContract, CallHedge.address, PutHedge.address)
      ...
      await deployer.deploy(CallHedge, CHL);
      await deployer.deploy(PutHedge, DAI, CHL, UNS);
      await deployer.deploy(HedgeContract, CallHedge.address, PutHedge.address);
